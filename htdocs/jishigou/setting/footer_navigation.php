<?php
 /* save by 1 127.0.0.1 2013-04-11 11:27:36 */

 $config['footer_navigation'] = array (
  'list' =>
  array (
    0 =>
    array (
      'order' => 101,
      'name' => '找感兴趣的人',
      'code' => 'findfriend',
      'url' => '#',
      'target' => '_parent',
      'avaliable' => 1,
      'type_list' =>
      array (
        0 =>
        array (
          'order' => 98,
          'name' => '名人堂',
          'code' => 'people',
          'url' => 'index.php?mod=people',
          'target' => '_parent',
          'avaliable' => '1',
        ),
        1 =>
        array (
          'order' => 97,
          'name' => '推荐用户',
          'code' => 'media',
          'url' => 'index.php?mod=other&code=media',
          'target' => '_parent',
          'avaliable' => '1',
        ),
        2 =>
        array (
          'order' => 96,
          'name' => '排行榜',
          'code' => 'charts',
          'url' => 'index.php?mod=top&code=member',
          'target' => '_parent',
          'avaliable' => '1',
        ),
        3 =>
        array (
          'order' => 95,
          'name' => '最新评论',
          'code' => 'maybefriend',
          'url' => 'index.php?mod=profile&code=maybe_friend',
          'target' => '_parent',
          'avaliable' => '1',
        ),
      ),
    ),
    1 =>
    array (
      'order' => 99,
      'name' => '精彩内容',
      'code' => 'content',
      'url' => '#',
      'target' => '_parent',
      'avaliable' => 1,
      'type_list' =>
      array (
        0 =>
        array (
          'order' => 98,
          'name' => '微直播',
          'code' => 'live',
          'url' => 'index.php?mod=live',
          'target' => '_parent',
          'avaliable' => '1',
        ),
        1 =>
        array (
          'order' => 97,
          'name' => '微访谈',
          'code' => 'talk',
          'url' => 'index.php?mod=talk',
          'target' => '_parent',
          'avaliable' => '1',
        ),
        2 =>
        array (
          'order' => 96,
          'name' => '官方推荐',
          'code' => 'recd',
          'url' => 'index.php?mod=plaza',
          'target' => '_parent',
          'avaliable' => '1',
        ),
        3 =>
        array (
          'order' => 96,
          'name' => '最新微博',
          'code' => 'newweibo',
          'url' => 'index.php?mod=topic&code=recd',
          'target' => '_parent',
          'avaliable' => '1',
        ),
      ),
    ),
    2 =>
    array (
      'order' => 90,
      'name' => '热门应用',
      'code' => 'app',
      'url' => '#',
      'target' => '_parent',
      'avaliable' => 1,
      'type_list' =>
      array (
        0 =>
        array (
          'order' => 100,
          'name' => '微博秀',
          'code' => 'show',
          'url' => 'index.php?mod=show&code=show',
          'target' => '_parent',
          'avaliable' => '1',
        ),
        1 =>
        array (
          'order' => 99,
          'name' => '图片墙',
          'code' => 'picwall',
          'url' => 'index.php?mod=topic&code=photo',
          'target' => '_parent',
          'avaliable' => '1',
        ),
        2 =>
        array (
          'order' => 98,
          'name' => '上墙',
          'code' => 'control',
          'url' => 'index.php?mod=wall&code=control',
          'target' => '_parent',
          'avaliable' => '1',
        ),
        3 =>
        array (
          'order' => 97,
          'name' => '图片签名档',
          'code' => 'qmd',
          'url' => 'index.php?mod=tools&code=qmd',
          'target' => '_parent',
          'avaliable' => '1',
        ),
      ),
    ),
    3 =>
    array (
      'order' => 8,
      'name' => '关于我们',
      'code' => 'about',
      'url' => '#',
      'target' => '_parent',
      'avaliable' => 1,
      'type_list' =>
      array (
        0 =>
        array (
          'order' => 1,
          'name' => '联系我们',
          'code' => 'link',
          'url' => 'index.php?mod=other&code=contact',
          'target' => '_parent',
          'avaliable' => '1',
        ),
        1 =>
        array (
          'order' => 1,
          'name' => '申请V认证',
          'code' => 'vip',
          'url' => 'index.php?mod=other&code=vip_intro',
          'target' => '_parent',
          'avaliable' => '1',
        ),
        2 =>
        array (
          'order' => 1,
          'name' => '备案申请中',
          'code' => 'beian',
          'url' => 'http://www.miibeian.gov.cn/',
          'target' => '_parent',
          'avaliable' => '1',
        ),
      ),
    ),
    4 =>
    array (
      'order' => 7,
      'name' => '手机玩微博',
      'code' => 'mobile',
      'url' => '#',
      'target' => '_parent',
      'avaliable' => 1,
      'type_list' =>
      array (
        0 =>
        array (
          'order' => 1,
          'name' => 'iPhone客户端',
          'code' => 'iphone',
          'url' => 'index.php?mod=other&code=iphone',
          'target' => '_parent',
          'avaliable' => '1',
        ),
        1 =>
        array (
          'order' => 1,
          'name' => 'Android客户端',
          'code' => 'android',
          'url' => 'index.php?mod=other&code=android',
          'target' => '_parent',
          'avaliable' => '1',
        ),
        2 =>
        array (
          'order' => 1,
          'name' => '3G网页',
          'code' => '3G',
          'url' => 'index.php?mod=other&code=mobile',
          'target' => '_parent',
          'avaliable' => '1',
        ),
        3 =>
        array (
          'order' => 1,
          'name' => 'WAP访问',
          'code' => 'wap',
          'url' => 'index.php?mod=other&code=wap',
          'target' => '_parent',
          'avaliable' => '1',
        ),
      ),
    ),
  ),
  'pluginmenu' =>
  array (
  ),
);
?>