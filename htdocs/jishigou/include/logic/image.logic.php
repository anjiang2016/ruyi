...
class ImageLogic
{
    ...
    public function remove_topic_image($id=0){
		...
		while ($value = DB::fetch($query)){
			...
			$value['description'] = $value['description'] ? $value['description'] : 'Í¼Æ¬¼ò½é';
	...
	function addalbum($name='') {
		...
		if($uid > 0 && $name && $name != 'ÇëÊäÈëÏà²áÃû³Æ' && $name != 'Ä¬ÈÏÏà²á') {
	...
	function getalbumname($aid=0,$listall=0,$url=1,$uid=0){
		...
		if($listall){
			...
			if($url){
				return '<a href="'.jurl('index.php?mod=album&code=list&uid='.$val['uid']).'">'.$val['nickname'].'µÄÏà²á</a> >> '.$val['albumname'];
			}else{
