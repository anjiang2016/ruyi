...
class TopicManageLogic
{
		...
		if(!$topic_list){
			return false;
				...
				if ($type == 1 && $topic_list['type'] == 'first') {
					...
					if ($score > 0) {
						$data = array('uid' => $topic_list['uid'], 'rid'=>0, 'relatedid'=>MEMBER_ID, 'dateline'=>time(),'remark'=>"·¢²¼Î¢²© ¡¾Î¢²©ID:{$weibo_href}¡¿");
	...
	function doForceOut($nickname_arr=array(),$cause="Ã»ÓÐÀíÓÉ",$role_id=4){
